...
/*
 ...
 customHelpers: module object with handlebars helpers (optional)
...
function render(source, config, onSuccess, onError) {
    ...
    // Register core handlebar helpers
    ...
    console.log("config.customHelpers: "+config.customHelpers);

    for (var helperName in config.customHelpers) {
        console.log("helperName: "+helperName);
        if (config.customHelpers.hasOwnProperty(helperName)) {
            handlebars.registerHelper(helperName, config.customHelpers[helperName]);
        }
    }
...
function getDefaultConfig(https, mainTemplate, resourceTemplate, itemTemplate, customHelpersFile) {
    ...
    var customHelpersModule = require(
        (customHelpersFile)
            ? (process.cwd()+'/'+customHelpersFile)
            : (__dirname + '/custom.helpers.js'));
    ...
    return {
        ...
        customHelpers: customHelpersModule,
        partials: {
...
if (require.main === module) {
    program
        ...
        .option('-h, --helpers [helpers]', 'Path to custom helpers.js module file')
        .parse(process.argv);
    ...
    render(input, getDefaultConfig(https, program.template, program.resource, program.item, program.helpers), function(result) {
        if (program.output) {
