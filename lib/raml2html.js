...
function _parsePath(templateParam, templateDefault) {
    if (templateParam) {
...
if (require.main === module) {
    ...
    var mainTemplate = _parsePath(program.template, './template.handlebars');
    var resourceTemplate = _parsePath(program.resource, './resource.handlebars');
    var itemTemplate = _parsePath(program.item, './item.handlebars');

    // Start the parsing process
    parseWithConfig(input, getDefaultConfig(https, mainTemplate, resourceTemplate, itemTemplate), function(result) {
        if (program.output) {
