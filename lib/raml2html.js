...
function _descriptionHelper(description) {
    if (description && description.length) {
        var index = description.indexOf(null);
        if (index !== -1) {
        	description.splice(index, 1);
        }

        if (description.length && description.indexOf("#vision") > -1) {
            return new handlebars.SafeString(' <span class="glyphicon glyphicon-eye-open">&nbsp;</span>');
        }
        
        if (description.length && description.indexOf("#development") > -1) {
            return new handlebars.SafeString(' <span class="glyphicon glyphicon-cog">&nbsp;</span>');
        }

        if (description.length && description.indexOf("#live") > -1) {
            return new handlebars.SafeString(' <span class="glyphicon glyphicon-star">&nbsp;</span>');
        }

        if (description.length && description.indexOf("#hot") > -1) {
            return new handlebars.SafeString(' <span class="glyphicon glyphicon-fire">&nbsp;</span>');
        }

        if (description.length && description.indexOf("#new") > -1) {
            return new handlebars.SafeString(' <span class="glyphicon glyphicon-leaf">&nbsp;</span>');
        }
    }
...
function _ifContains(v1, v2, options) {
	  if(v1 && v1.length && v1.indexOf(v2) > -1) {
...
function getDefaultConfig(https, mainTemplate, resourceTemplate, itemTemplate) {
    ...
    return {
        ...
        helpers: {
            ...
            desc: _descriptionHelper,
            ifTypeIsString: _ifTypeIsString,
            ifContains: _ifContains
        },
        partials: {
